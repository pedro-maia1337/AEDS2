...
class Hash {
   ...
   public boolean inserir(Game elemento) {
      ...
      if (elemento != null) {
         ...
         if (tabela[pos] == null) { // insere caso esteja vazia
            tabela[pos] = elemento.clone();
            resp = true;
         } else { // caso não, faz o rehash e tenta a inserção novamente
             pos = h2(elemento.getName());
             if(tabela[pos] == null) {
                 tabela[pos] = elemento.clone();
                 resp = true;
             }  
      ...
      return resp; //caso mesmo com o rehash não seja possível inserir, retorna falso
   }
